...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('match_substitutions', function (Blueprint $table) {
            ...
            $table->foreignId('outgoing_player_id')->constrained('users');
            $table->foreignId('incoming_player_id')->constrained('users');
            $table->enum('status', ['pending', 'approved', 'rejected'])->default('pending');
            $table->boolean('fee_paid')->default(false); // If the incoming player needed to pay
            $table->timestamps();
        });
    }
